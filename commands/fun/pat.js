...
exports.run = async (client, message, argsls) => { //lets started your commands script
  if (!message.mentions.users.first() || message.guild.members.cache.get(args[0])) return message.reply("you almost mention someone to pat them"); //if no one is mentions , lets reply as
